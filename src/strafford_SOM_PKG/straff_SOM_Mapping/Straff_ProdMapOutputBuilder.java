 ...
 *
 */
public class Straff_ProdMapOutputBuilder {
	...
	public Straff_ProdMapOutputBuilder(Straff_SOMMapManager _mapMgr, String _fileName, ExecutorService _th_exec, int _pDistType, double _pZnDistThresh) {
	...
	public void saveAllSpecifiedProdMappings() {//launch in either a single thread or multiples
		...
		if((isMT) && (numProdsToMap >= 2*numUsableThreads)) {//save multiple product mappings per thread
			...
			List<Straff_ProdOutMapper_Dist> prdcttMappers = new ArrayList<Straff_ProdOutMapper_Dist>();
			...
			for (int i=0; i<(numUsableThreads-1);++i) {				
				prdcttMappers.add(new Straff_ProdOutMapper_Dist(mapMgr, stIDX, endIDX, i, prodDistType, prodZoneDistThresh, prodsToMap,fullQualOutPerProdDirs));
				stIDX = endIDX;
				endIDX += numForEachThrd;
			}
			//last one probably won't end at endIDX, so use length
			prdcttMappers.add(new Straff_ProdOutMapper_Dist(mapMgr,stIDX, numProdsToMap, numUsableThreads-1, prodDistType, prodZoneDistThresh, prodsToMap,fullQualOutPerProdDirs));
			...
			Straff_ProdOutMapper_Dist mapper = new Straff_ProdOutMapper_Dist(mapMgr,0, prodsToMap.length, 0, prodDistType, prodZoneDistThresh, prodsToMap,fullQualOutPerProdDirs);
			mapper.call();
			
//			for (int i=0;i<prodsToMap.length;++i) {
//				ProductExample ex = prodsToMap[i];
//				ArrayList<String> strRes = ex.getAllExmplsPerProdStrAra(prodDistType,prodZoneDistThresh);
//				String OID = ex.OID;
...
//			}
	...
	public void saveAllProspectToProdMappings(SOMExample[] prospectsToMap, String typeOfProspect) {
		...
		if(isMT) {//save multiple product mappings per thread
			int numUsableThreads = mapMgr.getNumUsableThreads();
			List<Future<Boolean>> prdcttMapperFtrs = new ArrayList<Future<Boolean>>();
			List<Straff_ProspectOutMapper_Dist> prdcttMappers = new ArrayList<Straff_ProspectOutMapper_Dist>();
			int numForEachThrd = ((int)((prospectsToMap.length-1)/(1.0f*numUsableThreads))) + 1;
			//use this many for every thread but last one
			int stIDX = 0;
			int endIDX = numForEachThrd;				
			for (int i=0; i<(numUsableThreads-1);++i) {		
				prdcttMappers.add(new Straff_ProspectOutMapper_Dist(msgObj, stIDX, endIDX, i, typeOfProspect, prospectsToMap, prodToMapNodes, fullQualOutPerProspectDir));
				stIDX = endIDX;
				endIDX += numForEachThrd;
			}
			//last one probably won't end at endIDX, so use length
			prdcttMappers.add(new Straff_ProspectOutMapper_Dist(msgObj,stIDX, prospectsToMap.length, numUsableThreads-1, typeOfProspect, prospectsToMap, prodToMapNodes, fullQualOutPerProspectDir));
			try {prdcttMapperFtrs = th_exec.invokeAll(prdcttMappers);for(Future<Boolean> f: prdcttMapperFtrs) { f.get(); }} catch (Exception e) { e.printStackTrace(); }	
			
		} else {//save prospect 	
			//invoke single-threaded version synchronously
			Straff_ProspectOutMapper_Dist mapper = new Straff_ProspectOutMapper_Dist(msgObj,0, prospectsToMap.length, 0, typeOfProspect, prospectsToMap, prodToMapNodes, fullQualOutPerProspectDir);
			mapper.call();//			
//			
...
class Straff_ProdOutMapper_Dist implements Callable<Boolean>{
	private MessageObject msgObj;
	private FileIOManager fileIO;
	private ProductExample[] prodsToMap;
	...
	public Straff_ProdOutMapper_Dist(Straff_SOMMapManager _mapMgr, int _stIDX, int _endIDX, int _thdIDX, int _pDistType, double _pZnDistThresh, ProductExample[] _prodsToMap, String[] _fullQualOutDirs) {
...
class Straff_ProspectOutMapper_Dist implements Callable<Boolean>{
	private MessageObject msgObj;
	private FileIOManager fileIO;
	...
	public Straff_ProspectOutMapper_Dist(MessageObject _msgObj, int _stIDX, int _endIDX, int _thdIDX, String _prspctType,  SOMExample[] _prospectsToMap, HashMap<ProductExample, HashMap<SOMMapNode, Double>> _prodToMapNodes,String _fullQualOutDir) {
		msgObj = _msgObj;	
