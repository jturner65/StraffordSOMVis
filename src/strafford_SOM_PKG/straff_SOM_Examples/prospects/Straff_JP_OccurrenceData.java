 ...
 */
public class Straff_JP_OccurrenceData{
	...
	public Straff_JP_OccurrenceData(Integer _jp, Integer _jpg, boolean _usesOpt) {jp=_jp;jpg=_jpg; usesOpt=_usesOpt;occurrences = new TreeMap<Date, TreeMap<Integer, Integer>>();}	
	...
	public Straff_JP_OccurrenceData(String _csvString) {
	...
	private void parseStringToOccVals(String _csvStringRaw) {	
		...
		for(int j=1;j<occRecs.length;++j) {//skip first record
			...
			Date date = Straff_BaseRawData.buildDateFromString(optRecData[0]);
	...
	public String toCSVString() {
		...
		for (Date date : occurrences.keySet()) {
			res += perDateOccRecSt;
			String dateString = Straff_BaseRawData.buildStringFromDate(date);
			res += dateString+",";
			TreeMap<Integer, Integer> valsAtDate = occurrences.get(date);
			res +=""+valsAtDate.size()+",";
