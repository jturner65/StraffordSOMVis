...
public class CustProspectExample extends ProspectExample {
	...
	private void dbg_addOrderCountToTTLMap(TreeMap<Integer, JP_OccurrenceData> map) {	synchronized(ttlOrderCount) {	addOrderToOrderCountMap(map, ttlOrderCount);	}}
	private synchronized void dbg_addBadOrderCountToTTLMap(TreeMap<Integer, JP_OccurrenceData> map) {	synchronized(ttlBadOrderCount) {	addOrderToOrderCountMap(map, ttlBadOrderCount);}}
	...
	private void buildDateBasedOccsAndTrainingFtrData() {
		...
		for (Date orderDate : dateOrderEventOccs.keySet()) {// get order events and dates
			...
			Cust_OneOrderTrainingExample ex = new Cust_OneOrderTrainingExample(this, OID + "_" + String.format("%3d", exID++), orderOcc, dateEventOccurrences.headMap(orderDate, true));// true: includes events ondate
	...
	protected final void buildStdFtrsMap() {
		...
		if ((mapOfSingleOrderTrainingExamples != null) && (mapOfSingleOrderTrainingExamples.size() > 0)) {
			for (Cust_OneOrderTrainingExample ex : mapOfSingleOrderTrainingExamples.values()) {			ex.buildAfterAllFtrVecsBuiltStructs();		}
		}
	...
	private final void updateStaticVars(TreeMap<Integer, JP_OccurrenceData> map) {
	...
	public final void buildNonProdJpFtrVec() {
		if (nonProdJpgJps.size() > 0) {((Straff_SOMMapManager) mapMgr).ftrCalcObj.buildCustNonProdFtrVecs(this, ftrMaps[ftrMapTypeKey],nonProdJpgJps, JpOccurrences.get("sources"));}
	}
