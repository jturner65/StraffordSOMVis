 ...
 *
 */
public abstract class Straff_ProspectExample extends Straff_SOMExample{
	...
	protected TreeMap<String, TreeMap<Integer, Straff_JP_OccurrenceData>> JpOccurrences;
	...
	private Straff_JP_OccurrenceData posOptAllEventObj = null, negOptAllEventObj = null;
	...
	public Straff_ProspectExample(SOM_MapManager _map, SOM_ExDataType _type, Straff_ProspectData _prspctData) {
	...
	public Straff_ProspectExample(SOM_MapManager _map, SOM_ExDataType _type, String _OID, String _csvDataStr) {
	...
	//copy ctor
	public Straff_ProspectExample(Straff_ProspectExample _otr) {
		super(_otr);
	...
	protected StraffEvntRawToTrainData buildNewTrainDataFromEv(Straff_EventRawData _evntObj, Straff_EvtDataType eventtype) {
		switch (eventtype) {
		case Order 	: {		return new Straff_OrderEventRawToTrainData((Straff_OrderEvent) _evntObj);	}//order event object
		case Opt 	: {		return new Straff_OptEventRawToTrainData((Straff_OptEvent) _evntObj);}//opt event  object
		case Link 	: { 	return new Straff_LinkEventRawToTrainData((Straff_LinkEvent) _evntObj);}//link event
		case Source : { 	return new Straff_SrcEventRawToTrainData((Straff_SourceEvent) _evntObj);}//source event
		default : {			return new Straff_OrderEventRawToTrainData((Straff_OrderEvent) _evntObj);	}//default to order event object - probably will fail.  need to make sure type is accurately specified.
		}//switch
	}//buildNewTrainData
	...
	protected StraffEvntRawToTrainData buildNewTrainDataFromStr(Integer _evIDStr, String _evTypeStr, String _evDateStr, String _evntStr, Straff_EvtDataType eventtype) {
		switch (eventtype) {
		case Order 	: { 	return new Straff_OrderEventRawToTrainData(_evIDStr, _evTypeStr, _evDateStr, _evntStr);	}//order event object
		case Opt 	: {		return new Straff_OptEventRawToTrainData(_evIDStr, _evTypeStr, _evDateStr, _evntStr);}//opt event  object
		case Link 	: {		return new Straff_LinkEventRawToTrainData(_evIDStr, _evTypeStr, _evDateStr, _evntStr);}//link event  object
		case Source : {		return new Straff_SrcEventRawToTrainData(_evIDStr, _evTypeStr, _evDateStr, _evntStr);}//source event  object
		default : {			return new Straff_OrderEventRawToTrainData(_evIDStr, _evTypeStr, _evDateStr, _evntStr);	}//default to order event object - probably will fail.  need to make sure type is accurately specified.
		}//switch
	}//buildNewTrainData		
	 ...
	 */
	protected void addDataToTrainMap(Straff_EventRawData _optObj, TreeMap<Date, TreeMap<Integer, StraffEvntRawToTrainData>> map, Straff_EvtDataType type){		
	...
	protected final void buildJPOccTrainDataFromCSVStr(String jpOccType, int numOccs, String csvJPOccData) {
		...
		if(jpOccs == null) {jpOccs = new TreeMap<Integer, Straff_JP_OccurrenceData>();JpOccurrences.put(jpOccType, jpOccs);}
		String [] allOccsData = csvJPOccData.trim().split(Straff_JP_OccurrenceData.occRecStTag);
		...
		for(int i=1;i<allOccsData.length;++i) {
			Straff_JP_OccurrenceData occ = new Straff_JP_OccurrenceData(allOccsData[i]);
	...
	protected final void buildOccurrenceStructs(String[] eventMapTypeKeys, boolean[] jpOccMapUseOccData) { // should be executed when finished building all xxxEventsByDateMap(s)
		...
		for(int i=0;i<eventMapTypeKeys.length;++i) {
			...
			TreeMap<Integer, Straff_JP_OccurrenceData> occs = new TreeMap<Integer, Straff_JP_OccurrenceData>();
	...
	protected final TreeMap<String, TreeMap<Integer, Straff_JP_OccurrenceData>> copyJPOccStruct(String[] eventMapTypeKeys) {
		TreeMap<String, TreeMap<Integer, Straff_JP_OccurrenceData>> tmpJpOccurrences = new TreeMap<String, TreeMap<Integer, Straff_JP_OccurrenceData>> ();
	...
	private final String getRawDescrForCSV_JPOcc() {
		...
		String dateStr = Straff_BaseRawData.buildStringFromDate(prs_LUDate);
		...
		for(int i=0;i<occMapTypeKeys.length;++i) {
			...
			TreeMap<Integer, Straff_JP_OccurrenceData> occs = JpOccurrences.get(key);
			for(Straff_JP_OccurrenceData occ : occs.values()) {		res += occ.toCSVString();	}//for each occurrence of type key\
		}
		return res;		
	...
	public final HashSet<Tuple<Integer,Integer>> getSetOfAllJpgJpData(){
		...
		for(TreeMap<Integer, Straff_JP_OccurrenceData> occs : JpOccurrences.values()) {for (Straff_JP_OccurrenceData occ :occs.values()) {res.add(occ.getJpgJp());}	}
		return res;
	}///getSetOfAllJpgJpData
	
	//return occurence map
	public final TreeMap<Integer, Straff_JP_OccurrenceData> getOcccurenceMap(String key) {
		TreeMap<Integer, Straff_JP_OccurrenceData> res = JpOccurrences.get(key);
	...
	public final Straff_JP_OccurrenceData getPosOptAllOccObj() {		return posOptAllEventObj;	}
	public final void setPosOptAllOccObj(Straff_JP_OccurrenceData _optAllOcc) {posOptAllEventObj = _optAllOcc;	}
		
	public final Straff_JP_OccurrenceData getNegOptAllOccObj() {		return negOptAllEventObj;	}
	public final void setNegOptAllOccObj(Straff_JP_OccurrenceData _optAllOcc) {negOptAllEventObj = _optAllOcc;	}
	...
	protected synchronized final void buildJPListsAndSetBadExample() {
		...
		for(TreeMap<Integer, Straff_JP_OccurrenceData> occs : JpOccurrences.values()) {
			for (Integer jp :occs.keySet()) {
				...
				Straff_JP_OccurrenceData occ = occs.get(jp);
	...
	protected final void calcCompValMaps() {
		...
		if(nonProdJpgJps.size() > 0) {
			((Straff_SOMMapManager)mapMgr).ftrCalcObj.calcNonProdJpTrainFtrContribVec(this, nonProdJpgJps,compValFtrDataMaps[unNormFtrMapTypeKey], JpOccurrences.get("sources"));
			//build normalized and standardized comparison vector also
			if(compValFtrDataMapMag != 0.0) {
				for(Integer key : compValFtrDataMaps[unNormFtrMapTypeKey].keySet()) {	compValFtrDataMaps[perExNormMapTypeKey].put(key,  compValFtrDataMaps[unNormFtrMapTypeKey].get(key)/compValFtrDataMapMag);}	
				//use training example history to build std vector
				calcPerFtrNormVector(compValFtrDataMaps[unNormFtrMapTypeKey],  compValFtrDataMaps[perFtrNormMapTypeKey], mapMgr.getTrainFtrMins(), mapMgr.getTrainFtrDiffs());
			}			
		} 
	}//calcComValMaps()
	...
	protected String toStringOptOccMap(TreeMap<Integer, Straff_JP_OccurrenceData> map, String mapName) {
		String res = "\n# of jp occurences in " + mapName + " : "+ map.size()+"\n";	
		for (Integer jp : map.keySet() ) {
			Straff_JP_OccurrenceData occ = map.get(jp);
			res += occ.toString();			
		}	
		return res;		
	}//toStringOptOccMap
