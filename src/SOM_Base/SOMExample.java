...
//	}//checkForErrors
	...
	protected final void buildNghbrhdMapNodes(int _ftrType){
		...
		int mapCtrX = getBmu().mapNodeCoord.x, mapCtrY = getBmu().mapNodeCoord.y;
		Integer xTup, yTup;
	...
	protected final void buildNormFtrData() {
		if(!getFlag(ftrsBuiltIDX)) {mapMgr.msgObj.dispMessage("StraffSOMExample","buildNormFtrData","OID : " + OID + " : Features not built, cannot normalize feature data", MsgCodes.warning2);return;}
	...
	public final void buildFtrRprtStructs() {
		...
		for(Integer mapToGet : ftrMapTypeKeysAra) {//type of features
			...
			if(mapOfFtrsToIdxs == null) {mapMgr.msgObj.dispMessage("SOMExample" + OID,"buildFtrRprtStructs","Using inappropriate key to access mapOfWtsToFtrIDXs : " + mapToGet + " No submap exists with this key.", MsgCodes.warning2); return;}	
	...
	public final TreeMap<Integer,Integer> getMapOfFtrIDXBsWtRank(int mapToGet){
		if(!getFlag(ftrWtRptBuiltIDX)) {
			mapMgr.msgObj.dispMessage("SOMExample" + OID,"getMapOfFtrIDXBsWtRank","Feature-based report structures not yet built. Aborting.", MsgCodes.warning2);
			return null;
		}
	...
	public final TreeMap<Float, ArrayList<Integer>> getMapOfWtsToFtrIDXs(int mapToGet){
		if(!getFlag(ftrWtRptBuiltIDX)) {
			mapMgr.msgObj.dispMessage("SOMExample" + OID,"getMapOfWtsToFtrIDXs","Feature-based report structures not yet built. Aborting.", MsgCodes.warning2);
			return null;
		}
	...
	public double get_sqDistToBMU() {
		return _sqDistToBMU;
	}

	public void set_sqDistToBMU(double _sqDistToBMU) {
		this._sqDistToBMU = _sqDistToBMU;
	}

	public SOMMapNode getBmu() {
		return bmu;
	}

	public void setBmu(SOMMapNode bmu) {
		this.bmu = bmu;
	}
}//SOMExample 
